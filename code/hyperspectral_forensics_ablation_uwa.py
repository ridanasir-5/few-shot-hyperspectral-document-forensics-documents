...
# ============================================================
def load_ivisio_hhid_encoder(ckpt_path: str, in_ch: int,
...
def copy_ivisio_hhid_weights(iv_hhid_encoder: ResNet18Encoder,
                        tgt_encoder: BaseEncoder) -> None:
    """
    Copy all compatible weight tensors from iv_hhid_encoder into tgt_encoder.
    Used when a variant shares the same architecture as the baseline.
    """
    src_sd = iv_hhid_encoder.state_dict()
...
class AblationCondition:
    ...
    init:        str  = "ivisio_hhid_transfer"  # ivisio_transfer|imagenet|random|ivisio_frozen
...
def build_conditions() -> List[AblationCondition]:
    ...
    conditions = [
        # ── A: Backbone Architecture ─────────────────────────
        C("A","A1","ResNet18 (baseline)",     arch="resnet18",  init="ivisio_hhid_transfer"),
        ...
        C("B","B1","Full bands (baseline)",   band_key="full",      init="ivisio_hhid_transfer"),
        ...
        C("B","B4","Random 16 bands",         band_key="random16",  init="ivisio_hhid_transfer"),

        # ── C: Embedding Dimension ───────────────────────────
        C("C","C1","embed_dim=64",            embed_dim=64,  needs_train=True),
        C("C","C2","embed_dim=128",           embed_dim=128, needs_train=True),
        C("C","C3","embed_dim=256 (baseline)",embed_dim=256, init="ivisio_hhid_transfer"),
        C("C","C4","embed_dim=512",           embed_dim=512, needs_train=True),

        # ── D: Weight Initialisation Strategy ───────────────
        C("D","D1","iVision transfer (baseline)",  init="ivisio_hhid_transfer"),
        ...
        C("D","D4","iVision frozen backbone",       init="ivisio_hhid_frozen"),

        # ── E: Episodic Training Loss ────────────────────────
        C("E","E1","Proto sqL2 (baseline)",   loss_fn="proto_sqL2",   init="ivisio_hhid_transfer"),
        C("E","E2","Proto cosine",            loss_fn="proto_cosine", init="ivisio_hhid_transfer", needs_train=True),
        C("E","E3","Supervised Contrastive",  loss_fn="supcon",       init="ivisio_hhid_transfer", needs_train=True),
        C("E","E4","Batch-hard Triplet",      loss_fn="triplet",      init="ivisio_hhid_transfer", needs_train=True),
        ...
        # ── G: Embedding Normalisation ───────────────────────
        C("G","G1","L2-norm (baseline)", norm="l2",   init="ivisio_hhid_transfer"),
...
def run_condition(cond: AblationCondition,
                  df: pd.DataFrame,
                  iv_hhid_encoder: ResNet18Encoder,
                  train_items, test_items,
                  raw_in_ch: int,
                  device: torch.device,
                  n_way_cap: int = 5) -> Dict[str, float]:
    """
    ...
    if cond.init == "ivisio_hhid_transfer" and cond.arch == "resnet18":
        copy_ivisio_hhid_weights(iv_hhid_encoder, encoder)
        for p in encoder.parameters(): p.requires_grad_(True)
        print(f"    init: iVision transfer weights")

    elif cond.init == "ivisio_hhid_frozen" and cond.arch == "resnet18":
        copy_ivisio_hhid_weights(iv_hhid_encoder, encoder)
    ...
    if cond.needs_train:
        ...
        proxy_tr_acc = proxy_train(
            encoder, train_ds,
            n_way=min(n_way_cap, len(all_labels)-1),
            k_shot=min(cond.k_shot, 1),
...
def run_ablation(df: pd.DataFrame,
    ...
    train_items, test_items = build_crosssplit_items(
        cfg.ivisio_hhid_index if dataset_tag == "ivisio_hhid" else cfg.uwa_index,
        cfg.ivisio_hhid_dir   if dataset_tag == "ivisio_hhid" else cfg.uwa_dir,
...
def main():
    ...
    parser.add_argument("--dataset", default="ivisio_hhid",
        choices=["ivisio_hhid", "uwa", "both"],
    ...
    iv_hhid_enc   = load_ivisio_hhid_encoder(cfg.ivisio_hhid_ckpt, in_ch_ivisio_hhid)
    iv_hhid_enc.to(cfg.device)

    all_dataset_rows = []

    # ── iVision ablation ───────────────────────────────────────
    if args.dataset in ("ivisio_hhid", "both"):
        print("\n" + "#"*65)
        print("  ABLATION ON iVision HHID")
        print("#"*65)
        rows_ivisio_hhid = run_ablation(df_ivisio_hhid, iv_hhid_enc, in_ch_ivisio_hhid,
                               "ivisio_hhid", active_axes, cfg.out_dir)
        rows_ivisio_hhid["dataset"] = "ivisio_hhid"
        all_dataset_rows.append(rows_ivisio_hhid)

        print("\n  Generating iVision plots…")
        for axis in sorted(rows_ivisio_hhid["axis"].unique()):
            plot_axis(axis, rows_ivisio_hhid, plot_dir)
        plot_heatmap_all(rows_ivisio_hhid, plot_dir)
        plot_delta_from_baseline(rows_ivisio_hhid, plot_dir)
        plot_radar_full_vs_best(rows_ivisio_hhid, plot_dir)
        plot_sensitivity(rows_ivisio_hhid, plot_dir)
        save_results_table(rows_ivisio_hhid, cfg.out_dir, "ivisio_hhid")
    ...
    if args.dataset in ("uwa", "both"):
        ...
        if df_uwa is not None:
            ...
            print("#"*65)
            rows_uwa = run_ablation(df_uwa, iv_hhid_enc, in_ch_uwa,
    ...
    if len(all_dataset_rows) == 2:
        ...
        for ax_plot, met in zip(axes, ["writer_id_acc", "ink_mismatch_auc"]):
            for ds, color in [("ivisio_hhid","#1565C0"), ("uwa","#E65100")]:
